                ...
                @if($post->image)
                    <div class="overflow-hidden rounded-top">
                        <img src="{{ asset($post->image) }}" 
                             ...
                             alt="{{ $post->title }}" 
                             style="max-height:400px; object-fit:cover; transition: transform .4s;">
                    </div>
                @endif

        <div class="card-body">
            {{-- Blog Title --}}
            <h1 class="fw-bold text-dark blog-title">{{ $post->title }}</h1>
            
                        {{-- Meta Info --}}
                        <p class="text-muted mb-4">
                            ✍️ By <strong>{{ $post->author ?? 'JM Innovatech Solution' }}</strong> 
                            | 📅 {{ $post->created_at->format('d M, Y') }}
                        </p>
            
                        {{-- Blog Content --}}
                        <div class="blog-content" style="line-height:1.7; font-size:1.05rem; color:#333;">
                            {!! $post->content !!}
                        </div>
            ...
            <style>
                .blog-title {
                    font-size: 1.9rem;   /* Smaller than default H1 */
                    line-height: 1.3;
